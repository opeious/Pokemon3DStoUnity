...
namespace P3DS2U.Editor.SPICA.Converters
{
    public static class VerticesConverter
    {
        public static PICAVertex[] GetVertices (H3DMesh Mesh)
        {
            ...
            var Elems = new float[4];

            var Output = new PICAVertex[Mesh.RawBuffer.Length / Mesh.VertexStride];

            using (var MS = new MemoryStream (Mesh.RawBuffer)) {
                var Reader = new BinaryReader (MS);

                for (var Index = 0; Index < Output.Length; Index++) {
                    var Out = new PICAVertex ();
                    ...
                    foreach (var Attrib in Mesh.Attributes) {
                        AlignStream (MS, Attrib.Format);

                        for (var Elem = 0; Elem < Attrib.Elements; Elem++)
                            switch (Attrib.Format) {
                                case PICAAttributeFormat.Byte:
                                    Elems[Elem] = Reader.ReadSByte ();
                                    break;
                                case PICAAttributeFormat.Ubyte:
                                    Elems[Elem] = Reader.ReadByte ();
                                    break;
                                case PICAAttributeFormat.Short:
                                    Elems[Elem] = Reader.ReadInt16 ();
                                    break;
                                case PICAAttributeFormat.Float:
                                    Elems[Elem] = Reader.ReadSingle ();
                                    break;
                            }

                        var v = new Vector4 (Elems[0], Elems[1], Elems[2], Elems[3]);
                        ...
                        switch (Attrib.Name) {
                            ...
                            case PICAAttributeName.BoneIndex:
                                Out.Indices[bi++] = (int) v.X;
                                if (Attrib.Elements == 1) break;
                                Out.Indices[bi++] = (int) v.Y;
                                if (Attrib.Elements == 2) break;
                                Out.Indices[bi++] = (int) v.Z;
                                if (Attrib.Elements == 3) break;
                                Out.Indices[bi++] = (int) v.W;
                                break;
                    ...
                    if (Mesh.FixedAttributes != null) {
                        var HasFixedIndices = Mesh.FixedAttributes.Any (x => x.Name == PICAAttributeName.BoneIndex);
                        var HasFixedWeights = Mesh.FixedAttributes.Any (x => x.Name == PICAAttributeName.BoneWeight);

                        if (HasFixedIndices || HasFixedWeights)
                            foreach (var Attr in Mesh.FixedAttributes)
                                switch (Attr.Name) {
                                    case PICAAttributeName.BoneIndex:
            ...
            using (var MS = new MemoryStream ()) {
                ...
                foreach (var Vertex in Vertices) {
                    ...
                    foreach (var Attrib in Attributes) {
                        ...
                        for (var i = 0; i < Attrib.Elements; i++)
                            switch (Attrib.Name) {
                                case PICAAttributeName.Position:
                                    Write (Writer, Attrib, Vertex.Position, i);
                                    break;
                                case PICAAttributeName.Normal:
                                    Write (Writer, Attrib, Vertex.Normal, i);
                                    break;
                                case PICAAttributeName.Tangent:
                                    Write (Writer, Attrib, Vertex.Tangent, i);
                                    break;
                                case PICAAttributeName.Color:
                                    Write (Writer, Attrib, Vertex.Color, i);
                                    break;
                                case PICAAttributeName.TexCoord0:
                                    Write (Writer, Attrib, Vertex.TexCoord0, i);
                                    break;
                                case PICAAttributeName.TexCoord1:
                                    Write (Writer, Attrib, Vertex.TexCoord1, i);
                                    break;
                                case PICAAttributeName.TexCoord2:
                                    Write (Writer, Attrib, Vertex.TexCoord2, i);
                                    break;
                                case PICAAttributeName.BoneIndex:
                                    Write (Writer, Attrib, Vertex.Indices[bi++]);
                                    break;
                                case PICAAttributeName.BoneWeight:
                                    Write (Writer, Attrib, Vertex.Weights[wi++]);
                                    break;

                                default:
                                    Write (Writer, Attrib, 0);
                                    break;
                            }
                    }
                }
        ...
        private static void Write (BinaryWriter Writer, PICAAttribute Attrib, Vector4 v, int i)
        {
            switch (i) {
                case 0:
                    Write (Writer, Attrib, v.X);
                    break;
                case 1:
                    Write (Writer, Attrib, v.Y);
                    break;
                case 2:
                    Write (Writer, Attrib, v.Z);
                    break;
                case 3:
                    Write (Writer, Attrib, v.W);
                    break;
            }
        }
