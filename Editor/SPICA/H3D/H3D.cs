...
namespace SPICA.Formats.CtrH3D
{
    ...
    public class H3D : ICustomSerialization
    {
        ...
        public static H3D Open (MemoryStream MS)
        {
            ...
            var Deserializer = new BinaryDeserializer (MS, GetSerializationOptions ());

            var Header = Deserializer.Deserialize<H3DHeader> ();

            new H3DRelocator (MS, Header).ToAbsolute ();

            var Scene = Deserializer.Deserialize<H3D> ();
        ...
        public static void Save (string FileName, H3D Scene)
        {
            using (var FS = new FileStream (FileName, FileMode.Create)) {
                var Header = new H3DHeader ();

                var Relocator = new H3DRelocator (FS, Header);

                var Serializer = new BinarySerializer (FS, GetSerializationOptions ());

                var Contents = Serializer.Sections[(uint) H3DSectionId.Contents];
                 ...
                 */
                var CompStr = H3DComparers.GetComparisonStr ();
                var CompRaw = H3DComparers.GetComparisonRaw ();

                var Strings = new Section (0x10, CompStr);
                var Commands = new Section (0x80);
                var RawData = new Section (0x80, CompRaw);
                var RawExt = new Section (0x80, CompRaw);
                var Relocation = new Section ();
        ...
        private void AddUnique<T> (H3DDict<T> Src, H3DDict<T> Tgt) where T : INamed
        {
            ...
            foreach (var Value in Src) {
                ...
                while (Tgt.Contains (Name)) Name = $"{Value.Name}_{++Index}";
