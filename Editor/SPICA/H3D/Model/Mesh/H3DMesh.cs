...
namespace P3DS2U.Editor.SPICA.H3D.Model.Mesh
{
    ...
    public class H3DMesh : ICustomSerialization, ICustomSerializeCmd
    {
        ...
        public H3DMesh (IEnumerable<PICAVertex> Vertices, List<PICAAttribute> Attributes, ushort[] Indices) : this ()
        {
            ...
            if (Attributes == null) {
                Debug.LogError ("Attributes are null");
                return;
            }
        ...
        public H3DMesh (IEnumerable<PICAVertex> Vertices, List<PICAAttribute> Attributes,
            List<H3DSubMesh> SubMeshes) : this ()
        {
            H3DMeshImpl (Vertices, Attributes);
        ...
        {
            var Reader = new PICACommandReader (EnableCommands);
            ...
            while (Reader.HasCommand) {
                ...
                switch (Cmd.Register) {
                    ...
                    case PICARegister.GPUREG_ATTRIBBUFFER0_OFFSET:
                        BufferAddress = Param;
                        break;
                ...
                if (((BufferFormats >> (48 + Index)) & 1) != 0) {
                    ...
                    switch (Attrib.Name) {
                        case PICAAttributeName.Position:
                            Attrib.Scale = Reader.VtxShaderUniforms[7].X;
                            break;
                        case PICAAttributeName.Normal:
                            Attrib.Scale = Reader.VtxShaderUniforms[7].Y;
                            break;
                        case PICAAttributeName.Tangent:
                            Attrib.Scale = Reader.VtxShaderUniforms[7].Z;
                            break;
                        case PICAAttributeName.Color:
                            Attrib.Scale = Reader.VtxShaderUniforms[7].W;
                            break;
                        case PICAAttributeName.TexCoord0:
                            Attrib.Scale = Reader.VtxShaderUniforms[8].X;
                            break;
                        case PICAAttributeName.TexCoord1:
                            Attrib.Scale = Reader.VtxShaderUniforms[8].Y;
                            break;
                        case PICAAttributeName.TexCoord2:
                            Attrib.Scale = Reader.VtxShaderUniforms[8].Z;
                            break;
                        case PICAAttributeName.BoneWeight:
                            Attrib.Scale = Reader.VtxShaderUniforms[8].W;
                            break;
                    }
        ...
        bool ICustomSerialization.Serialize (BinarySerializer Serializer)
        {
            ...
            for (var Index = 0; Index < Attributes.Count; Index++) {
                ...
                switch (Attrib.Name) {
                    case PICAAttributeName.Position:
                        Scales[3] = Attrib.Scale;
                        break;
                    case PICAAttributeName.Normal:
                        Scales[2] = Attrib.Scale;
                        break;
                    case PICAAttributeName.Tangent:
                        Scales[1] = Attrib.Scale;
                        break;
                    case PICAAttributeName.Color:
                        Scales[0] = Attrib.Scale;
                        break;
                    case PICAAttributeName.TexCoord0:
                        Scales[7] = Attrib.Scale;
                        break;
                    case PICAAttributeName.TexCoord1:
                        Scales[6] = Attrib.Scale;
                        break;
                    case PICAAttributeName.TexCoord2:
                        Scales[5] = Attrib.Scale;
                        break;
                    case PICAAttributeName.BoneWeight:
                        Scales[4] = Attrib.Scale;
                        break;
                }
            }
            ...
            //Fixed Attributes
            foreach (var Attrib in FixedAttributes) {
            ...
            Writer.SetCommand (PICARegister.GPUREG_VSH_INPUTBUFFER_CONFIG, 0xa0000000u | (uint) (AttributesTotal - 1),
                0xb);
            ...
            for (var Index = 0; Index < FixedAttributes.Count; Index++) {
                var Attrib = FixedAttributes[Index];
        ...
        {
            var Params = Material.MaterialParams;

            var Quat = Attributes.Any (x => x.Name == PICAAttributeName.Normal);
            var VertA = Attributes.Any (x => x.Name == PICAAttributeName.Color);
            var BoneW = Attributes.Any (x => x.Name == PICAAttributeName.BoneWeight);

            var UVMap0 = Params.TextureCoords[0].MappingType == H3DTextureMappingType.UvCoordinateMap;
            var UVMap1 = Params.TextureCoords[1].MappingType == H3DTextureMappingType.UvCoordinateMap;
            var UVMap2 = Params.TextureCoords[2].MappingType == H3DTextureMappingType.UvCoordinateMap;

            var IsTex1 = Material.EnabledTextures[1];
            var IsTex2 = Material.EnabledTextures[2];

            if (BoneW) BoneW = Attributes.First (x => x.Name == PICAAttributeName.BoneWeight).Elements == 4;

            foreach (var SM in SubMeshes) {
                var IsSmoSk = SM.Skinning == H3DSubMeshSkinning.Smooth;
                var IsRgdSk = SM.Skinning == H3DSubMeshSkinning.Rigid;
