...
namespace P3DS2U.Editor.SPICA
{
    internal static class IOUtils
    {
        ...
        {
            var Remainder = Reader.BaseStream.Position % BlockSize;

            if (Remainder != 0) Reader.BaseStream.Seek (BlockSize - Remainder, SeekOrigin.Current);
        }

        public static void Align (this BinaryWriter Writer, int BlockSize, byte FillByte)
        {
            while (Writer.BaseStream.Position % BlockSize != 0) Writer.Write (FillByte);
        }
    }
}